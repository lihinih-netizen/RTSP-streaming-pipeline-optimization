...
def create_stability_chart(df, test_id, output_dir):
    ...
    axes[0].set_ylim(0, max(df['cpu_normalized']) * 1.2)  # Auto-scale instead of fixed 0-100
...
def create_group_chart(config_avg, group_name, group_info, output_dir):
    ...
    axes[0, 0].set_ylim(0, max(group_data['cpu_mean_normalized']) * 1.2)  # Auto-scale instead of fixed 0-100
    for bar, val in zip(bars, group_data['cpu_mean_normalized']):
        axes[0, 0].text(bar.get_x() + bar.get_width()/2, bar.get_height() + max(group_data['cpu_mean_normalized']) * 0.02,
    ...
    axes[1, 1].set_ylim(0, max(group_data['cpu_max_normalized']) * 1.2)  # Auto-scale instead of fixed 0-100
    for bar, val in zip(bars, group_data['cpu_max_normalized']):
        axes[1, 1].text(bar.get_x() + bar.get_width()/2, bar.get_height() + max(group_data['cpu_max_normalized']) * 0.02,
                        f'{val:.1f}%', ha='center', va='bottom', fontweight='bold')
...
def create_comparison_charts(all_stats, output_dir):
    ...
    axes[0, 0].grid(axis='y', alpha=0.3)
    axes[0, 0].set_ylim(0, max(config_avg['cpu_mean_normalized']) * 1.2)  # Auto-scale instead of fixed 0-100
    ...
    axes[1, 1].grid(axis='y', alpha=0.3)
    axes[1, 1].set_ylim(0, max(config_avg['cpu_max_normalized']) * 1.2)  # Auto-scale instead of fixed 0-100
